...
class AdFilterTest(unittest.TestCase):
    ...
    def testFilter(self):
        afilter = ad_filter.regist_filter(None, {"prefix": ["CUST"]})
        data = {
            "feed": {},
            "entries": [
                {"title": "ok title"},
        ...
        entries = feed["entries"]
        self.assertEquals(entries[0][ad_filter.AD_FILTER_KEY], 0)
        self.assertEquals(entries[1][ad_filter.AD_FILTER_KEY], 0)
        self.assertEquals(entries[2][ad_filter.AD_FILTER_KEY], 0)
        self.assertEquals(entries[3][ad_filter.AD_FILTER_KEY], 1)
        self.assertEquals(entries[4][ad_filter.AD_FILTER_KEY], 0)
